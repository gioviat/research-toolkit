...
- `literature-scout` (agent) — runs the survey: arXiv API, Semantic Scholar API, web search, snowballing; reports its queries so coverage is auditable.
...
- `theory-checker` (agent) — verdict per claim: correct / incorrect / unverified, never a general impression.
...
Everything is a markdown file with YAML frontmatter. The criteria are field-generic on purpose — which statistical tests are standard, which venues matter, what counts as a strong baseline all vary by subfield. Tighten the relevant `SKILL.md` or agent file for yours.
